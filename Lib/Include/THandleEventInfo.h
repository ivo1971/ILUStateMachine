...
namespace ILULibStateMachine {
   ...
   template <class TEventData> class THandleEventInfo : public CHandleEventInfoBase {
      public:
         /** Prototype of an event guard: depending on its return value the corresponding event handler is called.
          **/
         typedef bool                                              FGuard(void);                                                   
         /** Prototype of an event handler: it is provided with the event data.
          **/
         typedef void                                              FHandler(const TEventData* const pEventData);                   
         /** FGuard wrapped in a boost function, so the event guard can be a class method bound to a class instance.
          **/
         typedef boost::function<FGuard>                           BFGuard;                                                        
         /** FHandler wrapped in a boost function, so the event handler can be a class method bound to a class instance.
          **/
         typedef boost::function<FHandler>                         BFHandler;                                                      
         /** Type that fully defines one event action: guard (optional), handler, state transition. This maps 1-on-1 to 1 arrow in a state machine schema.
          **/
         typedef boost::tuple<BFGuard, BFHandler, CCreateState>    GuardHandlerCreateState;                                        
         /** Container of event action descriptors.
          **/
         typedef std::vector<GuardHandlerCreateState>              GuardHandlerCreateStates;                                       
         /** Iterator on the container of event action descriptors.
          **/
         typedef typename GuardHandlerCreateStates::iterator       GuardHandlerCreateStatesIt;                                     

         /** Constant iterator on the container of event action descriptors.
          **/
         typedef typename GuardHandlerCreateStates::const_iterator GuardHandlerCreateStatesCIt;                                    
