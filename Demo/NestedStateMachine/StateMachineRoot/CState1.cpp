...
namespace DemoNestedStateMachineRoot {
   namespace Internal {
      void CState1::HandlerEvt1(const DemoNestedStateMachineEvents::CEventRootData* const pData)
      {
         ILULibStateMachine::LogInfo("%s %s data [%d]\n", GetName().c_str(), __FUNCTION__, pData->Get());
         m_pData->m_strMsg = "CState1::HandlerEvt1 called";
      }
